...
class Env:
    ...
    def __init__(self):
        ...
        self.num_lasers = 24
        self.state_num = self.num_lasers + 6 #  2 for linear & angular vel, 2 for distance and angle diff + 2 env variables
        ...
        self.x_init = [6.0]#[0.0, 2.0] # [0.0,4.0]
        self.y_init = [6.0]#[0.0, -2.0]  # [0.0,0.0]
        self.x_prev = [6.0]#[0.0, 2.0] # [0.0,4.0]
        self.y_prev = [6.0]#[0.0, -2.0]  # [0.0,0.0]
        ...
        # Set target position
        self.target = [[2.0, 2.0]]#[[4.0, 0.0], [2.0, 2.0]] # Two goal (crossing scenario) # [[4.0,0.0], [0.0,0.0]]
    ...
    def reset(self, model_state = None, id_bots = 999):
        ...
        for i in range(self.num_robots):
            ...
            robot_msgs[i].pose.orientation.x = 0.0#self.robot_quat[i][0]
            robot_msgs[i].pose.orientation.y = 0.0#self.robot_quat[i][1]
            robot_msgs[i].pose.orientation.z = -0.2#self.robot_quat[i][2]
            robot_msgs[i].pose.orientation.w = 0.0#self.robot_quat[i][3]
    ...
    def getLaser(self, scan, daynight):
        scan_range = []
        min_range = 0.25
        collision = False
        for i in range(len(scan.ranges)):
            if scan.ranges[i] == float('Inf'):
                ...
                if daynight == 1: # Day condition
                    scan_range.append(scan.ranges[i])
                else:
                    gaussian_noise = np.random.normal(0, 0.15)

                    sensor_night = scan.ranges[i] + gaussian_noise
                    sensor_night = min(3.5, max(0, sensor_night))
                    print("sensor_pure: {}".format(scan.ranges[i]))
                    print("sensor_night: {}".format(sensor_night))
                    scan_range.append(sensor_night)
    ...
    def step(self, actions, time_step=0.1):
        ...
        twists = [self.action_to_twist(action) for action in np.asarray(actions).reshape(1,2)]
        ...
        print("distance_to_goals: {}".format(distance_to_goals))
        ...
        # 4. Assign Environmental state
        daytime_flag = [0] # 0 - night, 1 - day
        season_flag = [1]  # 0 - autumn, 1 - summer

        # 5. Read LaserSensor (state) from the robot's position
        ...
        for i in range(self.num_robots):
            ...
            laser, collision[i] = self.getLaser(scan[i], daytime_flag[i])
            state.append(laser)
        ...
        state_arr = np.hstack((state_arr, np.asarray(daytime_flag).reshape(self.num_robots,1)))
        state_arr = np.hstack((state_arr, np.asarray(season_flag).reshape(self.num_robots,1)))
