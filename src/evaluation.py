...
def main(args):
	...
	weight_path = HOME + '/catkin_ws/src/frl_swarm/src/evaluation/'
	choice = args.algo
	if choice == 1:
		algo_name = 'IACDDPG'
		exp_name = '/20210528-130516/'
	elif choice == 2:
		algo_name = 'SNACDDPG'
		exp_name = '/20210528-234257/'
	elif choice == 3:
		algo_name = 'SEACDDPG'
		exp_name = '/20210529-145838/'
	elif choice == 4:
		algo_name = 'FLDDPG'
		exp_name = '/20210525-155924/'
	random.seed(args.random_seed)

	########################################################
	env= eval_env.Env(algo_name)
	...
	if train_indicator==0:
		for i in range(num_trials):
			print("trial:" + str(i))
			_, current_states = env.reset() 
			path = weight_path + algo_name + exp_name
			epoch_num = "990"
			net_size = "256"
			
			agent.actor_model.load_state_dict(torch.load('{}'.format(path)+'actormodel'+'-{}-{}.pkl'.format(epoch_num, net_size)))
			agent.critic_model.load_state_dict(torch.load('{}'.format(path)+'criticmodel'+'-{}-{}.pkl'.format(epoch_num, net_size)))
			##############################################################################################
			total_reward = 0
			...
			for j in range(trial_len):
				...
				action, eps = agent.act(current_states)
				action = action.reshape((1, env.action_space.shape[0]))
				actions.append(action)
				
				_, new_states, rewards, dones, infos, is_stops, isEverSuccess = env.step(action, 0.1) # we get reward and state here, then we need to calculate if it is crashed! for 'dones' value
				###########################################################################################
