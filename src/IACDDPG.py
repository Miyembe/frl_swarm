...
def main(args):
	...
	logger_ins = logger.Logger(HOME + '/catkin_ws/src/frl_swarm/src/log/IACDDPG', output_formats=[logger.HumanOutputFormat(sys.stdout)])
	board_logger = tensorboard_logging.Logger(os.path.join(logger_ins.get_dir(), "tf_board", time_str))
	random.seed(args.random_seed)
	log_path = HOME + '/catkin_ws/src/frl_swarm/src/log/IACDDPG/csv/'
	weight_path = HOME + '/catkin_ws/src/frl_swarm/src/weights/IACDDPG/'
	########################################################
	env= ppswarm_env.Env()#turtlebot_env.Env()
	...
	if (train_indicator==1):
		...
		for i in range(num_trials):
			...
			for j in range(trial_len):
				...
				print("Time for one step: {}".format(end_time - start_time))
				##########################################################################################
			if (i % 10==0):
				for k in range(num_robots):
			...
			if i % log_interval == 0 or i == 0:
				...
				for j in range(num_robots):
					logger_ins.logkv('eprewmean_{}'.format(j), reward_idvl_mean[j][0])
					board_logger.log_scalar('eprewmean_{}'.format(j), reward_idvl_mean[j][0], i)
				...
				for j in range(env.num_robots):
					with open(log_path+ '{}_{}.csv'.format(agents[j].file_name, j), mode='a') as csv_file:
						csv_writer = csv.writer(csv_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
						csv_writer.writerow(['%i'%i, '%0.2f'%reward_idvl_mean[j][0]])
				reward_idvl_buf = []
