...
def main(args):
	...
	logger_ins = logger.Logger(HOME + '/catkin_ws/src/frl_swarm/src/log/PPDDPG', output_formats=[logger.HumanOutputFormat(sys.stdout)])
	board_logger = tensorboard_logging.Logger(os.path.join(logger_ins.get_dir(), "tf_board", time_str))
	random.seed(args.random_seed)
	log_path = HOME + '/catkin_ws/src/frl_swarm/src/log/PPDDPG/csv/'
	weight_path = HOME + '/catkin_ws/src/frl_swarm/src/weights/PPDDPG/'
	...
	toBeUpdated = [[None] for _ in range(env.num_robots)]
	...
	if (train_indicator==1):
		...
		for i in range(num_trials):
			...
			for j in range(trial_len):
				...
				_, new_states, rewards, dones, infos, is_stops, isEverSuccess, toBeUpdated, partnerIDs = env.step(toBeUpdated, actions, 0.1) # we get reward and state here, then we need to calculate if it is crashed! for 'dones' value
				...
				for k in range(num_robots):
					if toBeUpdated[k] == 1:
						target_agents = [agents[k], agents[partnerIDs[k]]]
						actor_mean_weights, actor_mean_bias, critic_mean_weights, critic_mean_bias = get_averaged_weights(target_agents, 2, num_actor_layers, num_critic_layers)
						...
						update_agents_models(target_agents, models_dict, num_actor_layers, num_critic_layers)
						toBeUpdated[k] = 0 
						print("{} and {} shared weights!".format(k, partnerIDs[k]))

				print("1 time step: {}".format(time.time() - time_step_start))
