	...
	if (train_indicator==1):
		...
		for i in range(num_trials):
			...
			if i % weight_interval == 0:
				if idx_robot < env.num_robots-1:
					agents_wa = [agents[idx_robot], agents[idx_robot+1]]
					print("robot {} and robot {} share weights.".format(idx_robot, idx_robot+1))
					idx_robot += 1
				else:
					agents_wa = [agents[idx_robot], agents[0]]
					print("robot {} and robot {} share weights.".format(idx_robot, 0))
					idx_robot = 0
				actor_mean_weights, actor_mean_bias, critic_mean_weights, critic_mean_bias = get_averaged_weights(agents_wa, 2, num_actor_layers, num_critic_layers)
				...
				soft_update_agents_models(agents, models_dict, num_actor_layers, num_critic_layers, 0.5)
